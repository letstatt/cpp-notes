...
Стоит отметить, что использование этой конструкции с аргументами функций запрещено, потому что они не являются `constexpr`-выражением (?). Поэтому данная конструкция не является заменой обычному `if`.
